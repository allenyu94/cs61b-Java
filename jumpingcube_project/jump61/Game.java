...
class Game {
    ...
    /** Updates currplayer to the correct player. */
    private void updateplayer() {
        if (currplayer.getColor() == redPlayer.getColor()
                && currplayer != redPlayer) {
            currplayer = redPlayer;
        } else if (currplayer.getColor() == bluePlayer.getColor()
    ...
    private void setMoveNumber(int n) {
        ...
        _playing = false;
        _board.setMoves(n);
        if (currplayer.getColor() != _board.whoseMove()) {
            if (currplayer.getColor() == RED) {
                currplayer = bluePlayer;
            } else {
                currplayer = redPlayer;
            }
